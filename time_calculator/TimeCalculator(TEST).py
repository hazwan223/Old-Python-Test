def add_time(start, duration, starting_day=None):
    ...
    print("--------------------------------")
    print(f"extraHalfDay: {extra_half_day}")
    print(f"Hour: {hour}")
    print(f"ExtraDay: {extra_day}")
    print(f"12 hour: {hour}:{minute}")
    print("--------------------------------")
    ...
    if not extra_half_day % 2 == 0:  # if odd
        if period == "AM":
            period = "PM"
        else:
            period = "AM"
